...
public class Chain : MonoBehaviour
{
    ...
    void updateHindleJoints2D()
    {
        ...
        for (int i = 0; i < links.Count; i++)
        {
            ...
            if (p.hingleJoint_2 != null) DestroyImmediate(p.hingleJoint_2, true);
    ...
    public void SetColor(Color color)
    {
        for (int i = 0; i < links.Count; i++)
        {
            links[i].obj.GetComponent<SpriteRenderer>().color = color;
        }
    }
    ...
    public void SetLayer(int layer)
    {
        for (int i = 0; i < links.Count; i++)
        {
            links[i].obj.layer = layer;
        }
    }

    public void resetPositions(Vector3 posA, Vector3 dir)
    {
        for (int i = 0; i < links.Count; i++)
        {
            LinkInfo p = links[i];
            ...
            p.obj.transform.right = dir;
            Rigidbody2D rigidbody = p.obj.GetComponent<Rigidbody2D>();
